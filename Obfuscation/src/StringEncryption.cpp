...
static cl::opt<int>
    ObfuTimes("enc-str-times", cl::init(1),
...
void StringEncryptionPass::insertIntDecryption(Module &M, EncryptedGV encGV, ConstantInt *intData) {
    vector<Type *> args;
    ...
    LoadInst *val = builder.CreateLoad(intData->getType(), encGV.GV, "");
    Value *xorVal =
...
void StringEncryptionPass::insertArrayDecryption(Module &M, EncryptedGV encGV, ConstantDataArray *arrData) {
    vector<Type *> args;
    FunctionType *funcType =
        FunctionType::get(Type::getVoidTy(M.getContext()), args, false);
    string funcName = genHashedName(encGV.GV);
    ...
    Value *ele = builder.CreateGEP(arrData->getType(), encGV.GV, ArrayRef<Value *>(indexList, 2));
    ArrayType *arrTy = cast<ArrayType>(encGV.GV->getValueType());
    Type *eleTy = arrTy->getElementType();
    Value *encEle =
        builder.CreateXor(builder.CreateLoad(arrData->getElementType(), ele), CONST(eleTy, encGV.key));
